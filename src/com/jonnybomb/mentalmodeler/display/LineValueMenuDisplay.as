package com.jonnybomb.mentalmodeler.display
{
	...
	public class LineValueMenuDisplay extends Sprite
	{
		...
		private function drawIcon(h:int, w:int):Sprite {
			...
			g = icon.graphics;
			//CMapConstants.LINE_COLOR_POSITIVE:uint = 0x0351A6;
			//CMapConstants.LINE_COLOR_NEGATIVE:uint = 0xBF5513;
			g.beginFill(CMapConstants.LINE_COLOR_NEGATIVE, 0.3);
			var top:Number = 0;
			var bot:Number = h;
			var left:Number = 0;
			...
			g.endFill();
			g.beginFill(CMapConstants.LINE_COLOR_POSITIVE, 0.3);
			g.moveTo(midRight, mid);
			g.lineTo(midRight, top);
			g.lineTo(left, top);
			g.lineTo(midLeft, mid);
			g.endFill();
			t = 2;
